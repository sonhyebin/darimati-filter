...
- **라이브에서는 세그멘테이션을 돌리지 않는다** (Kirakira와 같은 구조). 프리뷰는
  원본 영상이 박스 안에 들어간 상태고, 누끼는 셔터를 누른 정지 사진에서만 딴다.
  라이브 추론이 없으니 저사양 폰에서도 프리뷰가 매끄럽고 배터리를 아낀다
- 촬영 시 누끼(`captureHQ`)는 Kirakira의 전경→인물→원본 폴백을 그대로 따른다:
  1. **MediaPipe Tasks** selfie_multiclass (GPU→CPU) — 머리카락/피부/옷 클래스 인식
  2. 마스크가 비거나 실패하면 구형 Selfie Segmentation (2021)
  3. 둘 다 실패하면 누끼 없이 원본 사진 그대로
  파이프라인: 전체 프레임 추론 → 사람 bbox 크롭 재추론(모델이 사람을 크게 봄) →
  guided filter로 실제 이미지 엣지에 스냅 + smoothstep 밴드(`HQ_LO/HI`)로 halo 제거
- 두 엔진 모두 카메라를 켜는 동안 백그라운드에서 미리 로드된다
